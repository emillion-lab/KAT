...
1. History drew risk_by_weekday / risk_by_month from data/historical.json —
   pre-recalibration numbers (Sat 1.24, Mon 0.92) contradicting what the engine
   now uses (Sat 0.905, Mon 1.074). A dashboard disagreeing with its own model
   is worse than no dashboard.
2. The 3-day tab still printed a crash count. Out-of-sample R2 is 0.44, so a
   point estimate is a promise the model cannot keep.
3. Kp, moon and pressure rendered as ordinary factor cards, so they read as
   contributors. They stay — showing they were checked is worth more than
   hiding them — but under an explicit heading.
"""
import io
...
# ---------------- 2. HISTORY heading + observations -------------------------
...
rep("""    `Сезонен пик: <b>${moNames[topMo]}</b> (+${Math.round((moMax-1)*100)}%) — отпускарски трафик и умора.`,""",
...
# ---------------- 3. METHODOLOGY block appended to History ------------------
...
/* Kp, Луна и Налягане остават видими — че са проверени, е част от работата —
   но не бива да изглеждат като фактори. */
function markObservedOnly(){
  ...
  document.querySelectorAll('.factor').forEach(el=>{
    ...
    if(labels.some(l=>t.includes(l))){
      ...
      const bar=el.querySelector('.factor-bar-wrap');
...
/* Как е построен моделът: какво влезе, какво отпадна и какво не знаем.
   Пестеливо: подробностите изглеждат несериозно, премълчаването — нечестно. */
function methodologyBlock(bg){
  ...
  <div style="background:var(--surf);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:12px">
    ...
    ${S_('▸ <b>Лунни фази</b> — проверени в три държави. Фазите се разминават: България и САЩ са почти в противофаза. Отхвърлено.')}
    ${S_('▸ <b>Налягане</b>, <b>обезводняване в жега</b>, <b>витамин D</b>, <b>заслепяване при залез</b>, <b>планетарни цикли</b> — нито едно не оцелява след контрол за календара и времето.')}
    ${S_('Оставени са като информация на началната страница, за да се вижда, че са били проверени.')}
    ${head('КАКВО ЗНАЕМ, НО НЕ МОЖЕМ ДА ОБЯСНИМ')}
    ${S_('Аварийността има <b>памет от около седмица</b>: ако последните дни са били тежки, следващите също. Затихва наполовина за 5 дни, изчезва към десетия.')}
    ${S_('Потвърдена е и в трите държави, включително върху американските смъртни катастрофи, които не зависят от полицейско отчитане. През зимата е пет пъти по-силна.')}
    ${S_('<b>Причината е неизвестна.</b> Не е цикъл — няма период, който се повтаря. Проверени и отхвърлени: партидно вкарване на данни, остатъчно време, работен цикъл, медийно отразяване.')}
    ${head('ГРАНИЦИ НА ТОЧНОСТТА')}
    ${S_('Моделът обяснява <b>44%</b> от дневната промяна, проверено върху години, които не е виждал. Една тежка катастрофа мести числото повече от всички фактори заедно.')}
